...
using stdpar_ranges_execution_policy = std::execution::parallel_policy;

constexpr auto stdpar_md_range(stdpar_ranges_execution_policy, int M, int N) {
  return std::views::cartesian_product(std::views::iota(0, M), std::views::iota(0, N));
}

constexpr auto stdpar_md_range(stdpar_ranges_execution_policy, int M, int N, int P) {
  return std::views::cartesian_product(std::views::iota(0, M), std::views::iota(0, N), std::views::iota(0, P));
}
  ...
  const global_const_arrays<MemorySpace>& arrays)
{
...

  {
    ...
    std::for_each(exec_policy, begin(range), end(range),
      [state](int ll, int k) {
  ...
  int k_beg)
{
  ...
  auto range = stdpar_md_range(exec_policy, nz + 2*hs);
  std::for_each(exec_policy, begin(range), end(range),
    [=] (int k) {
      // OpenACC doesn't support static arrays, even if they are constexpr.
      constexpr int nqpoints = 3;
  ...
  auto range = stdpar_md_range(exec_policy, nz + 1);
  std::for_each(exec_policy, begin(range), end(range),
    [=] (int k) {
...
struct stdpar_reducer {
  ...
  reduction_result local_result(std::tuple<const int&, const int&> k_i_pair) const {
    auto [k, i] = k_i_pair;
  ...
  reduction_result
  operator() (std::tuple<const int&, const int&> k_i_pair, const reduction_result& thread_local_result) const {
    return (*this)(local_result(k_i_pair), thread_local_result);
  }

  reduction_result
  operator() (const reduction_result& thread_local_result, std::tuple<const int&, const int&> k_i_pair) const {
    return (*this)(thread_local_result, local_result(k_i_pair));
  }

  reduction_result
  operator() (std::tuple<const int&, const int&> p0, std::tuple<const int&, const int&> p1) const {
    return (*this)(local_result(p0), local_result(p1));
  }
};
  ...
  const global_const_arrays<MemorySpace>& const_arrays)
{
  ...
  auto reducer = stdpar_reducer{state, hy_dens_cell, hy_dens_theta_cell};
  result = std::reduce(exec_policy, begin(range), end(range), result, reducer);
  return result;
}
